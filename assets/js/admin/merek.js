...
function postData(){
  ...
  if(validator.form()){
    ...
    // $.post('url', data);
    var api = base_url+"admin/merek/post";
    
    $.post(api, data).done(function( data ) {
      if (data == "1"){
        bootbox.alert("Data merek berhasil disimpan.", function(){
          location.reload();
        })
      }else{
...
$(document).on('click','.delete',function(){
  ...
  bootbox.confirm("Apakah anda yakin akan menghapus data merek "+$(tr).find('td').eq(1).html()+" - "+$(tr).find('td').eq(2).html()+"? ", function(result){
    if (result) {
      $.post( base_url+"admin/merek/delete", {id : idx}).done(function( data ) {
        if (data == '1'){
...
$(document).on('click','.update',function(){
  ...
  var tr = $(this).closest('tr');
  $('#active').show();
  $.post( base_url+"admin/merek/getById", {idx: idx}).done(function( data1 ) {
    var json = $.parseJSON(data1);
    console.log(json)
    $('[name="nama"]').val(json.nama);
    $('[name="id_merek"]').val(json.id_merek);
